...
class DummyOptimizer(dobject):
    ...
    def exitstep(self, fx, u0, x):
        ...
        print "Energy differences", np.absolute((fx - u0) / self.beads.natoms) , self.tolerances["energy"]
        print "MaxForces" , np.amax(np.absolute(self.forces.f)) , self.tolerances["force"]
        print "Force Difference" , np.linalg.norm(self.forces.f.flatten() - self.old_f.flatten()) , 1e-08
        print "Position" , x, self.tolerances["position"]
